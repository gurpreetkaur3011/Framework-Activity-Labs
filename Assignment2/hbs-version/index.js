...
app.get("/fetch-multiple", (req, res) => {
  try {
    ...
    for (let i = 0; i < numImages; i++) {
      selectedImages.push(fetchRandomImage());
    }

    res.json(selectedImages);
  } catch (error) {
    console.error("Error fetching multiple random images:", error.message);
    res.status(503).send({
      message: "Error fetching multiple random images",
      error: error.message,
    });
  }
});
...
app.get("/gallery", (req, res) => {
  ...
  fs.readdir(uploadDir, (err, files) => {
    ...
    let imagePaths = imageFiles.map((file) => path.join("/uploads", file));
    res.render("gallery", { title: "Gallery", files: imagePaths });
  });
});
